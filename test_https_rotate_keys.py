...
VAULT_TOKEN = "root"
VAULT_CACERT = "/tmp/vault-tls1064275867/vault-ca.pem"  # Adjust if needed
...
def wrap_import_key_material(target_key: bytes, wrapping_key_pem: str) -> str:
    ...
    Steps:
      ...
      3. Load the Vault wrapping key (PEM) and wrap the ephemeral key using RSA OAEP with SHA-256.
      4. Concatenate the RSA-wrapped ephemeral key (expected to be the first 512 bytes) and the AES-wrapped key material.
    ...
    # 4. Concatenate the RSA-wrapped ephemeral key and the AES-wrapped target key.
...
def get_latest_key_version(key_name: str) -> int:
    """
    Retrieve the latest version of an existing key from Vault.
    """
    ...
    if response.status_code == 200:
        ...
        print(f"Latest version for key '{key_name}' is {latest_version}.")
        return latest_version
    else:
        raise Exception(f"Failed to retrieve key info: {response.text}")

def import_key_version(key_name: str, wrapped_key_material: str):
    """
    Imports a new version of an existing imported key into Vault's Transit engine.
    This uses the /transit/keys/<name>/import_version endpoint.
    """
    # Compute the new version number (latest version + 1)
    latest_version = get_latest_key_version(key_name)
    new_version = latest_version + 1

    url = f"{VAULT_ADDR}/v1/transit/keys/{key_name}/import_version"
    data = {
        "ciphertext": wrapped_key_material
    }
    response = requests.post(url, json=data, headers=HEADERS, verify=VAULT_CACERT)
    if response.status_code in [200, 204]:
        print(f"✅ Key '{key_name}' updated successfully to version {new_version}.")
    else:
        print(f"❌ Key version import failed: {response.text}")
...
if __name__ == "__main__":
    # For demonstration, assume target_key is your externally generated key material (e.g., 32 bytes)
    # Replace this with your actual key material if available.
    ...
    # Wrap the target key material using the wrapping key from Vault
    ...
    # Enter the name of the existing key you want to update.
    key_name = input("Enter the name of your existing imported key: ")
    
    # Import a new version of the key using the wrapped key material.
    import_key_version(key_name, wrapped_key_material)
    
    # Optionally, test encryption/decryption with the updated key.
